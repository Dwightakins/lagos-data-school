...
import { BookOpen, Award, Download, PlayCircle, Clock, Radio, Calendar, Film } from "lucide-react";
...
interface EnrolledCourse {
  ...
  nextLive: { lessonId: string; zoomLink: string; schedule: string | null } | null;
  lastRecording: { lessonId: string; title: string } | null;
}
    ...
    const load = async () => {
      ...
      const [enrollResult, certsResult] = await Promise.all([
        fetch("/api/student/courses", { cache: "no-store" })
          .then(async (r) => (r.ok ? (await r.json() as { enrollments: unknown[] }).enrollments : []))
          .catch(() => []),
        supabase
          .from("certificates")
          .select("course_id")
      ...
      const list = enrollResult as unknown as EnrolledCourse[];
            ...
            return (
              <div key={enr.id} className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col hover:shadow-lg hover:border-brand/20 transition-all duration-200">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  {/* Next live class */}
                  {enr.nextLive && (
                    <div className="mb-3 rounded-xl border border-amber-300/60 bg-amber-50 dark:bg-amber-900/15 dark:border-amber-700/40 px-3.5 py-2.5">
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wide mb-1">
                        <Radio className="w-3 h-3" /> Next Live Class
                      </div>
                      {enr.nextLive.schedule && (
                        <p className="flex items-center gap-1.5 text-[12px] text-amber-800 dark:text-amber-300 mb-2">
                          <Calendar className="w-3 h-3 shrink-0" />{enr.nextLive.schedule}
                        </p>
                      )}
                      <a
                        href={enr.nextLive.zoomLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-[12px] font-bold text-amber-700 dark:text-amber-400 hover:underline"
                      >
                        <Radio className="w-3 h-3" /> Join Zoom →
                      </a>
                    </div>
                  )}
                  ...
                  <div className="mt-auto pt-3 border-t border-border flex flex-col gap-2">
                    ...
                    {enr.lastRecording && (
                      <Link
                        href={`/learn/${enr.course_id}/${enr.lastRecording.lessonId}`}
                        className="flex items-center justify-center gap-2 border border-border text-muted-foreground hover:text-foreground hover:border-brand/30 font-semibold text-[12.5px] py-2 rounded-xl transition-colors"
                      >
                        <Film className="w-3.5 h-3.5" /> Watch Last Recording
                      </Link>
                    )}
